...
const dbPath = path.join(__dirname, 'data', 'dj-rotation.db');
...
`);
console.log('✅ Table DJHistory créée/vérifiée');
...
// Fonction pour générer l'URL YouTube
function generateYoutubeUrl(artist, title) {
...
function generateId(date, index) {
  return `ch${String(index).padStart(2, '0')}_${date}`;
}
...
historyData.forEach((entry, index) => {
  const id = generateId(entry.date, index + 1);
  const youtubeUrl = generateYoutubeUrl(entry.artist, entry.title);
  const playedAt = `${entry.date} 12:00:00`;

  try {
    const result = insertStmt.run(id, entry.dj, entry.title, entry.artist, youtubeUrl, playedAt);
    if (result.changes > 0) {
      console.log(`✅ ${entry.date} | ${entry.dj} | ${entry.artist} - ${entry.title}`);
      inserted++;
    } else {
      console.log(`⏭️  ${entry.date} | ${entry.dj} (existe déjà)`);
      skipped++;
    }
  } catch (err) {
    console.error(`❌ Erreur pour ${entry.date}: ${err.message}`);
  }
});

// Compter le total
const countResult = db.prepare('SELECT COUNT(*) as count FROM DJHistory').get();

console.log('\n📈 Résumé:');
console.log(`   - Entrées insérées: ${inserted}`);
console.log(`   - Entrées ignorées: ${skipped}`);
console.log(`   - Total dans la base: ${countResult.count}`);
