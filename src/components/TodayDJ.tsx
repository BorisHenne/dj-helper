...
export default function TodayDJ({ onSessionUpdated, onRequestSpin }: TodayDJProps) {
  ...
  const fetchTodaySession = async () => {
    try {
      ...
      // Fetch today's session with cache disabled
      ...
      if (!data.isBusinessDay) {
        ...
        // Si pas de prochaine session trouvée, continuer avec le flow normal
        setIsBusinessDay(false)
        setSession(null)
        setIsLoading(false)
        return
  ...
  // Si ce n'est pas un jour ouvrable et pas de prochaine session planifiée
      ...
      {!showingNextSession && (
        <div className="space-y-4">
          ...
          <div className="relative flex items-center">
            ...
            <span className="flex-shrink mx-4 text-gray-500 text-xs uppercase">{t('common.or')}</span>
            <div className="flex-grow border-t border-white/10"></div>
          </div>
