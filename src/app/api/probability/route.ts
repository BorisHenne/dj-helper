...
export async function GET() {
  try {
    const allDjs = await db.select().from(djs).where(sql`${djs.isActive} = 1`)
    ...
    // Separate active and on-leave DJs
    const availableDjs = djsWithHistoryData.filter(dj => !dj.isOnLeave)
    const onLeaveDjs = djsWithHistoryData.filter(dj => dj.isOnLeave)
    ...
    const djsWithProbability = calculateProbabilities(availableDjs, {
      weightLastPlayed: settingsRow?.weightLastPlayed ?? 0.6,
      weightTotalPlays: settingsRow?.weightTotalPlays ?? 0.4,
    })

    // Return on-leave DJs with 0 probability for display
    const onLeaveDjsWithProb = onLeaveDjs.map(dj => ({
      ...dj,
      probability: 0,
      daysSinceLastPlay: dj.lastPlayedAt
        ? Math.floor((Date.now() - new Date(dj.lastPlayedAt).getTime()) / (1000 * 60 * 60 * 24))
        : 365,
    }))

    return NextResponse.json({
      djs: djsWithProbability,
      onLeaveDjs: onLeaveDjsWithProb,
      settings: {
        weightLastPlayed: settingsRow?.weightLastPlayed ?? 0.6,
        weightTotalPlays: settingsRow?.weightTotalPlays ?? 0.4,
      },
...
export async function POST(request: NextRequest) {
  try {
    ...
    let query = db.select().from(djs).where(
      sql`${djs.isActive} = 1 AND ${djs.isOnLeave} = 0`
    )

    if (excludeDjIds.length > 0) {
      query = db.select().from(djs).where(
        sql`${djs.isActive} = 1 AND ${djs.isOnLeave} = 0 AND ${djs.id} NOT IN (${sql.raw(excludeDjIds.map(id => `'${id}'`).join(','))})`
      ) as typeof query
    }
